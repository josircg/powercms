...
from cms.models import Section, Article, ArticleComment, Recurso, ArticleAttribute
...
@register.simple_tag(takes_context=True)
def get_section_articles(context, slug, num=5, ordering=None):
    articles = []
    try:
        section = Section.objects.get(slug=slug)
        if ordering:
            articles_section = section.get_articles().order_by(ordering)
        else:
            articles_section = section.get_articles()
...
def site_name():
    try:
        r = Recurso.objects.get(recurso='SITE_NAME')
        return r.valor
    except Recurso.DoesNotExist:
        return f'{settings.SITE_NAME}'


@register.simple_tag
def site_setting(parameter):
    try:
        return getattr(settings, parameter, None)
    except:
        return None


@register.simple_tag
def site_parameter(parameter):
    try:
        return Recurso.objects.get(recurso=parameter).valor
    except Recurso.DoesNotExist:
        return ''
...
@register.simple_tag(takes_context=True)
def get_images(context, album_id, start=0):
    images = []
    for image in Image.objects.filter(album=album_id, order__gte=start).order_by('order'):
        images.append(image)
    return images
