...
def sendmail(subject='', to=[], params={}, template='', mimetype='text/html; charset=UTF-8', headers={}):
    ...
    def send_thread_email(subject='', to=[], params={}, template='', mimetype='text/html; charset=UTF-8', headers={}):
        from cms.models import EmailAgendado
...
def resendmail_email_agendado(email, mimetype='text/html; charset=UTF-8', headers={}):
    ...
    try:
        headers.update({'Reply-To': settings.REPLY_TO_EMAIL, })
        to_list = ast.literal_eval(email.to)
        if len(to_list) == 1:
            msg = EmailMultiAlternatives(email.subject, email.html, settings.DEFAULT_FROM_EMAIL, to=to_list, headers=headers)
        else:
            msg = EmailMultiAlternatives(email.subject, email.html, settings.DEFAULT_FROM_EMAIL, bcc=to_list, headers=headers)
