...
from utils.logentry import LogEntryMixin
...
class ThemeForm(LogEntryMixin, forms.ModelForm):
    class Meta:
        model = Theme
        fields = '__all__'
...
class PowerArticleForm(forms.ModelForm):
    ...
    link = forms.ChoiceField(label='Cadastro de link?', initial='True', choices=(('True', 'Sim'), ('False', 'Não')), widget=AdminRadioSelect(attrs={'class': 'radiolist inline'}))
    upload = forms.ImageField(label="Imagem", required=False)
    section = forms.ModelChoiceField(label='Seção', required=False, queryset=Section.objects.all())
...
class UpdateForm(forms.Form):
    ...
    version = forms.CharField(label='Versão', required=False,
                              help_text='Deixe em branco para atualizar para a ultima versão.', initial='master')
...
class CMSUserCreationForm(UserCreationForm):
    ...
    def save(self, commit=True):
        ...
        today = date.today()
        if not EmailAgendado.objects.filter(subject='Novo usuário solicitou o registro no site',
                                            date__year=today.year, date__month=today.month,
                                            date__day=today.day).exists():
            menssagem = 'Novo usuário solicitou o registro no site. Clique <a href="%s%s?is_active__exact=0">aqui</a> para visualizar.' % (settings.SITE_HOST, reverse('admin:auth_user_changelist'))
            sendmail(
                subject='Novo usuário solicitou o registro no site',
...
class CustomImageForm(forms.ModelForm):
    class Meta:
        widgets = {
            'title': forms.Textarea(attrs={'rows': '4', 'cols': '60', 'maxlength': '255'})
        }