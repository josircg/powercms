...
from .models import Article, Section, URLMigrate, FileDownload, Recurso, Permissao, \
    GroupType, GroupItem, EmailAgendado, URLNotFound, SectionItem, create_backup
...
class ArticleDetailView(DetailView):
    ...
    def get(self, *args, **kwargs):
        ...
        if self.request.user.is_authenticated and self.request.user.is_superuser or self.request.user.groups.filter(
                name=u'Editor').exists():
            html_content += u'''
...
def status(request):
    ...
    except EmailAgendado.DoesNotExist:
        pass
    json['mail'] = {
...
def page_not_found_view(request, exception=None):
    ...
    migrate = URLMigrate.objects.filter(old_url=request.path_info).first()
    if migrate:
        migrate.views += 1
        migrate.save()
        # isso não funcionou
        # if migrate.redirect_type == 'M':
        #    return HttpResponsePermanentRedirect(url.new_url)
        # return HttpResponseRedirect(url.new_url)
...
class ImportArticle(LoginRequiredMixin, TemplateView):
    ...
    def post(self, form):
        ...
        try:
            ...
            return JsonResponse(
                {'is_valid': False, 'msg': {'text': 'Arquivo inválido %s' % str(erro), 'type': 'danger'}})
        try:
            for article in articles:
                    ...
                    if not os.path.exists(path_og_image):
                        try:
                            ...
                            logs.append({'file': name, 'slug': '#',
                                         'c': path_og_image, 'is_include': True, 'type': 'Imagem',
                                         'add': 'glyphicon glyphicon-ok'})
                        except:
                            None
                    else:
                        nao_incluidas.append(name)
                        logs.append({'file': name, 'slug': '#',
                                     'c': path_og_image, 'is_include': False, 'type': 'Imagem',
                ...
                for img in imgs:
                    ...
                    if not is_path:  # not existir
                        try:
                            ...
                            logs.append({'file': name, 'slug': '#',
                                         'c': img.get('path'), 'is_include': True, 'type': 'Imagem',
                                         'add': 'glyphicon glyphicon-ok'})
                        except:
                            None
                    else:
                        nao_incluidas.append(name)
                        logs.append({'file': name, 'slug': '#',
                                     'c': img.get('path'), 'is_include': False, 'type': 'Imagem',
                                     'add': 'glyphicon glyphicon-remove'})
                ...
                logs.append({'file': obj_article.title, 'slug': obj_article.slug,
                             'c': '/admin/cms/article/%s/change/' % obj_article.pk, 'type': 'Artigo',
                             'add': 'glyphicon glyphicon-ok'})
            ...
            return JsonResponse({'is_valid': True, 'host': self.request.META['HTTP_ORIGIN'], 'logs': logs,
                                 'msg': {'text': 'Artigo adicionado com sucesso', 'type': 'success'}})
    ...
    def is_path(self, image):
        ...
        for image in images:
            ...
            if path.startswith('/media'):
                if os.path.exists(settings.PROJECT_DIR + image['src']):
...
class Backup(LoginRequiredMixin, View):
    def get(self, request):
        b = create_backup()
        messages.warning(request, mark_safe(
            'Backup Iniciado. <a href="/admin/cms/backup/%s">Clique aqui</a> para fazer o download.' % b.pk))
