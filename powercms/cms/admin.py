...
from .models import Menu, Section, Article, SectionItem, URLMigrate, \
    ...
    Theme, Permissao, GroupType, GroupItem, ArticleAttribute, URLNotFound, Backup, GroupVariable

from .forms import ThemeForm, CustomGroupForm, PowerArticleForm, CustomImageForm
...
from imagestore.admin import InlineImageAdmin

from imagestore.admin import AlbumAdmin, ImageAdmin
...
class FileDownloadAdmin(PowerModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        # Verifica se existe alguma seção onde os outros arquivos estão associados
        arquivo_padrao = FileDownload.objects.exclude(create_article__isnull=True).first()
        ...
        if obj.create_article and not obj.article:
            ...
            article.og_title = obj.get_absolute_url()
            article.save()
            obj.article = article
            obj.save()

            if arquivo_padrao:
                secao_padrao = arquivo_padrao.article.sections.first()
                if secao_padrao:
                    SectionItem.objects.create(section=secao_padrao, article=article,
                                               order=secao_padrao.articles.count()+1)
...
class GroupVariableInline(admin.TabularInline):
    model = GroupVariable
    extra = 1
...
class GroupTypeAdmin(PowerModelAdmin):
    ...
    inlines = [GroupItemGroupTypeInline, GroupVariableInline]
...
class ArticleAdmin(PowerModelAdmin):
    ...
    prepopulated_fields = {'slug': ('title',)}
    fieldsets = (
        (None, {'fields': [('title', 'slug'), 'header', 'content', 'keywords', 'created_at', 'author',
                           ('is_active', 'plain_header', 'allow_comments',),
                          ]}),
    ...
    def export_article(self, request, queryset):
        ...
        # Cria a pasta uploads/backup senao existir
        if not os.path.exists(os.path.join(settings.MEDIA_ROOT, 'uploads', 'backup')):
        ...
        for article in queryset:
            if article.content:
                soup = BeautifulSoup(article.content, features='html5lib')
        ...
        artigos = serializers.serialize(queryset, indent=4, use_natural_foreign_keys=True,
                                        fields=('title', 'slug', 'header', 'content', 'author', 'keywords',
                                                'is_active', 'og_title', 'og_image', 'plain_header'))
        # include article
        ...
        section_json = serializers.serialize(Section.objects.filter(id__in=unique_sections),
                                             indent=4, use_natural_foreign_keys=True,
                                             fields=('title', 'slug', 'header', 'keywords', 'order', 'template',
                                                     'views', 'conversions', ))
        info = zipfile.ZipInfo()
        ...
        info.external_attr = (0o644 << 16)
        info.date_time = datetime.now().timetuple()
        zip.writestr(info, section_json)

        section_json = serializers.serialize(SectionItem.objects.filter(section_id__in=unique_sections),
                                             indent=4, use_natural_foreign_keys=True,
                                             fields=('section__slug', 'article__slug', 'order'))
        info = zipfile.ZipInfo()
        info.filename = "items.json"
        info.compress_type = zipfile.ZIP_DEFLATED
        info.external_attr = (0o644 << 16)
        info.date_time = datetime.now().timetuple()
        zip.writestr(info, section_json)
        ...
        FileDownload.objects.get_or_create(title=article.title,
                                           file=os.path.join('uploads/backup',  name_file+'.zip'),
                                           article=article)
    ...
    export_article.short_description= u'Exportar Artigos'
    ...
    def backup(self, request, object_id):
        ...
        try:
            ...
            if not achou_arquivo:
                raise Exception('Nenhum tema foi encontrado no caminho: '+theme.path)
            zip.close()
            buffer.flush()
            ...
            LogObject(theme,'Backup efetuado para o tema %s' % theme.name, request.user)
            return response
        except Exception as e:
            messages.error(request, u'Erro ao criar .zip do tema %s' %(e))
...
class CustomInlineImageAdmin(InlineImageAdmin):
...
class CustomAlbumAdmin(AlbumAdmin):
    fieldsets = ((None, {'fields': ['name', 'brief', 'order', 'is_public']}),)
    list_display = ('name', 'admin_thumbnail', 'created', 'updated', 'order', 'is_public')
    list_editable = ('order', )
    inlines = [CustomInlineImageAdmin]
...
class CustomImageAdmin(ImageAdmin):
    fieldsets = ((None, {'fields': ['title', 'image', 'description', 'order', 'tags', 'album']}),)
    list_display = ('admin_thumbnail', 'order', 'album', 'title', 'width', 'height')
    list_filter = ('album', )
    form = CustomImageForm
...
class BackupAdmin(PowerModelAdmin):
    list_display = ('dtcriacao', 'backup', 'status',)
    readonly_fields = ('backup', 'status', 'dtcriacao',)
    change_list_template = 'admin/cms/backup_change_list.html'

    def get_buttons(self, request, object_id):
        buttons = super(BackupAdmin, self).get_buttons(request, object_id)
        if not object_id:
            buttons.append(PowerButton(url='/utils/backup', label="Adicionar Backup"))
        return buttons
