...
class Article(models.Model):
    ...
    header = models.TextField('Chamada', null=True, blank=True)
    ...
    keywords = models.TextField('Palavras Chaves', null=True, blank=True)
    created_at = models.DateField('Dt.Criação', default=timezone.now)
    updated_at = models.DateField('Dt.Alteração', auto_now=True)
    is_active = models.BooleanField('Está ativo?', default=True)
    allow_comments = models.CharField('Comentários', max_length=1, choices=ARTICLE_COMMENTS_CHOICES, default='N')
    views = models.BigIntegerField('Visualizações', default=0)
    conversions = models.BigIntegerField('Conversões', default=0)
    likes = models.BigIntegerField('Gostei', default=0)
    search = models.TextField(null=True, blank=True)
    plain_header = models.BooleanField(default=False)

    # Meta tags
    og_title = models.CharField('og-titulo', max_length=250, blank=True, null=True)
    og_image = models.ImageField('og-imagem', upload_to='articles', blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        if self.plain_header:
            self.header = unescape(Format.remove_tag(text=self.header))
        self.search = '%s %s %s %s' % (
            self.title,
    ...
    def get_sections_display(self):
        if self.sections.exists():
            return '%s' % self.sections.all()[0]
        return ' - '

    get_sections_display.short_description = 'Seção'
...
RECURSOS = (
    ('EMAIL', 'Envio de email automático'),
    ('SITE_NAME', 'Nome do Site'),
    ('ROBOTS', 'Permite busca pelo Google'),
    ('COMMENT_P', 'Texto para comentários privados'),
    ('COMMENT', 'Texto para comentários'),
    ('SIGNUP', 'Permite cadastro de usuários'),
    ('EMAILADMIN', 'Quem recebe avisos de novos usuários'),
    ('ANALYTICS', 'Google Analytics'),   # Deve ser adicionado ao base.html do tema, ao final do <head>
    ('CAPTCHA_PU', 'RECAPTCHA_PUBLIC_KEY'),
    ('CAPTCHA_PR', 'RECAPTCHA_PRIVATE_KEY'),
    ('OG-IMAGE', 'Imagem default para redes sociais'),
    ('TAGS', 'Nuvem de Tags'),
    ('TAGS-EXC', 'Excluir tags da núvem de tags (separar por virgula)'),
    ('TAGS-FIXA', 'Tags Fixas (separar por virgula)'),
    ('FACEBOOK', 'Facebook app id'),
    ('SUP_SLUG', 'Código no supervisord' )
)
...
class Theme(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            ...
            if ant.path_name and ant.path_name != self.path_name:
        ...
        if self.active:
            ...
            try:
                os.system("rm %s" % themedir)
            except Exception as e:
                LogObject(self, 'Tema atual não foi removido: %s' % e)
                return
            ...
            try:
                ...
                LogObject(self, 'Link para o Tema não foi criado. Erro: %s' % e)
                return

            # Reinicia o serviço
            try:
                slug = Recurso.objects.get(recurso='SERVICE_SLUG').valor
                os.system(f'supervisorctl restart {slug}&')  # Reinicia o serviço
            except Exception as e:
                LogObject(self, 'Serviço não foi reinicializado')


def remove_theme_path_delete(sender, instance, **kwargs):
    try:
        if settings.MEDIA_ROOT in instance.path:
            shutil.rmtree(instance.path)
            LogObject(instance, 'Pasta do Tema %s removida')
    except Exception as e:
        LogObject(instance, e.__str__)
...
def backup_upload_to(self, nome_arquivo):
    nome, extensao = os.path.splitext(nome_arquivo)
    return os.path.join('backup', str(uuid.uuid4()) + extensao.lower())
...
class Backup(models.Model):
    ...
    STATUS_BACKUP = (
        ('P', u'Processando...'),
        ('C', u'Concluído!'),
        ('E', u'Erro ao gerar backup!'),
    )

    def _backup_upload_to(self, nome_arquivo):
        nome, extensao = os.path.splitext(nome_arquivo)
        return os.path.join('backup', str(uuid.uuid4()) + extensao.lower())

    dtcriacao = models.DateTimeField(u'Dt. de criação', auto_now_add=True)
    backup = models.FileField(u'Backup', upload_to=backup_upload_to, blank=True, null=True)
    status = models.CharField(u'Status', max_length=1, choices=STATUS_BACKUP, default='P')



@receiver(signals.pre_delete, sender=Backup)
def backup_remove_arquivos(sender, instance, **kwargs):
    if instance.backup:
...
def create_backup():
    ...
    def create_backup_thead(backup):
        ...
        try:
            # Dump sql
            sqlfilename = 'db.sql'
            sqlfile = os.path.join(settings.MEDIA_ROOT,'uploads/backup',sqlfilename)
            # Ex.: 'mysqldump -h localhost -u root -p powercms > powecms.sql'
            os.system('mysqldump -h %s -u %s -p%s %s > %s' % (settings.DB_HOST, settings.DB_ROOT_USER, settings.DB_ROOT_PASSWORD, settings.DB_NAME, sqlfile))
        except Exception as e:
            backup.status = 'E'
            backup.save()
            return

        zip.write(sqlfile, sqlfilename)

        # incluindo arquivos
        media_path = os.path.join(settings.MEDIA_ROOT, 'uploads')
        for root, dirs, files in os.walk(media_path):
            for arquivo in files:
                zip.write(os.path.join(root, arquivo), os.path.join(root.replace(media_path, 'media'), arquivo))

        zip.close()
        temp.flush()
        ret_zip = temp.getvalue()
        temp.close()
        ...
        backup.status = 'C'
        backup.backup.save(backup._backup_upload_to('backup.zip'), ContentFile(ret_zip))
        backup.save()

    th = Thread(target=create_backup_thead, args=(backup, ))
    th.start()
    return backup